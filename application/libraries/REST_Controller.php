...
abstract class REST_Controller extends MY_Controller {
    ...
    public function _remap($object_called, $arguments) {
        ...
        if (config_item('rest_enable_limits') AND ! $this->_check_limit($controller_method)) {
            $this->response(array('status' => false, 'error' => 'Este usuario ha llegado al limite de intentos por hora para este metodo.'), 401);
        }
    ...
    protected function _check_limit($controller_method) {
        ...
        if (!$result OR $result->hour_started < time() - (60 * 60)) {
            ...
            $this->rest->db->insert(config_item('rest_limits_table'), array(
                'uri' => $this->uri->uri_string(),
                'api_key' => isset($this->user_id) ? $this->user_id : '',
                'count' => 1,
                'hour_started' => time()
            ));
        }
    ...
    protected function _check_login($username = '', $password = NULL) {
        ...
        $user_id = $this->authentication->check_login($username, $password);
        if ($user_id) {
    ...
    protected function _check_whitelist_auth() {
        ...
        if (!in_array($this->input->ip_address(), $whitelist)) {
            $this->response(array('estado' => 'error', 'error' => 'Acceso no autorizado'), 401);
        }
    }
