...
function updateResultadosProductos(){
    ...
    form.append('<input type="hidden" name="solo_ofertados" value="true">');
...
function bind_producto_row(){
    $('#tabla-resultados-left').find('.producto-ofertado').on('click',function(){        
        $('#pagina_tab2').val('1');
...
function updateResultadosDetalles(producto_id){        
    ...
    $.ajax({
        type: "POST",
        url: SITE_URL + 'panel_vendedor/ofertas/ajax_get_oferta_detalles',
        data: {
